...
def probe_boundaries_v3(
    ...
    out = cfg.output_path / "boundary_probes_v3.pt"
    shard_dir = cfg.output_path / "_probe_v3_shards"
    ...
    if accelerator.is_main_process:
        ...
        for layer, arrays in sorted(merged_features.items()):
            ...
            print(f"[v3 probe] layer={layer} acc={probes[layer]['accuracy']:.4f} eps={probes[layer]['epsilon']:.4f}")
        ...
        torch.save(
            {
                ...
                "note": "v3 uses matched safe prompts derived from each harmful instruction",
            },
            out,
        )
        (cfg.output_path / "boundary_probes_v3_meta.json").write_text(
            json.dumps(
                {
