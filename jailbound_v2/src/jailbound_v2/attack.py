...
def hotflip_suffix(
    ...
    for step in range(max(0, v2.text_attack.steps)):
        ...
        for pos, token_id in enumerate(token_ids):
            ...
            top_ids = torch.topk(scores, k=min(v2.text_attack.hotflip_top_k, scores.numel())).indices.tolist()
...
def optimize_sample_v2(
    ...
    suffix, hotflip_trace = hotflip_suffix(model, sample, probes, cfg, v2, initial_suffix)
...
def run_attack_v2(
    ...
    if accelerator.is_main_process and resume:
        print(
            f"[v2 attack][resume] completed={len(completed_rows)} pending={len(pending_indexed)} total={len(indexed)}",
            flush=True,
        )
        ...
        for local_pos, (global_index, sample) in enumerate(local_indexed, start=1):
            ...
            row = {
                ...
                "attack": {
                    ...
                    "text_attack": asdict(v2.text_attack),
                    **text_meta,
                },
            }
    ...
    if accelerator.is_main_process:
        ...
        print(f"merged {len(rows)} v2 attack rows from {accelerator.num_processes} process(es): {out_path}")
    accelerator.wait_for_everyone()
    return out_path
