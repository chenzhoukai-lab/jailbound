...
def hotflip_suffix(
    ...
    for step in range(max(0, v3.text_attack.steps)):
        ...
        for pos, token_id in enumerate(token_ids):
            ...
            top_ids = torch.topk(scores, k=min(v3.text_attack.hotflip_top_k, scores.numel())).indices.tolist()
            for candidate_id in top_ids:
                ...
                if best is None or candidate_score > best["score"]:
                    best = {"pos": pos, "old": int(token_id), "new": int(candidate_id), "score": candidate_score}
        if best is None:
            break

        new_ids = list(token_ids)
        new_ids[best["pos"]] = best["new"]
        suffix = _decode_clean(tokenizer, new_ids)
        trace.append(
            {
                "step": step,
                "loss": float(loss.cpu()),
                ...
                "span_meta": span_meta,
                "position": best["pos"],
                "old_token_id": best["old"],
                "new_token_id": best["new"],
                "old_token": tokenizer.decode([best["old"]], skip_special_tokens=False),
                "new_token": tokenizer.decode([best["new"]], skip_special_tokens=False),
                "suffix": suffix,
            }
        )
...
def optimize_sample_v3(
    ...
    if v3.text_attack.steps > 0:
        suffix, hotflip_trace = hotflip_suffix(model, sample, probes, cfg, v3, initial_suffix)
    else:
        suffix = initial_suffix
...
def run_attack_v3(
    ...
    cfg.validate_paths()
    boundary_path = Path(boundary_path or (cfg.output_path / "boundary_probes_v3.pt"))
    probes = load_probes(boundary_path)
    attack_variant = "jailbound_v3_hotflip" if v3.text_attack.steps > 0 else "jailbound_v3_suffix_bank"
    ...
    if accelerator.is_main_process and resume:
        print(
            f"[v3 attack][resume] completed={len(completed_rows)} pending={len(pending_indexed)} total={len(indexed)}",
            flush=True,
        )
        ...
        for local_pos, (global_index, sample) in enumerate(local_indexed, start=1):
            ...
            row = {
                ...
                "attack": {
                    ...
                    "text_attack": asdict(v3.text_attack),
                    **text_meta,
                },
            }
    ...
    if accelerator.is_main_process:
        ...
        print(f"merged {len(rows)} v3 attack rows from {accelerator.num_processes} process(es): {out_path}")
    accelerator.wait_for_everyone()
    return out_path
