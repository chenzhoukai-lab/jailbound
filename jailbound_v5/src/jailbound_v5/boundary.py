...
from .prompt_pairs import build_prompt_pairs, load_generated_safe_prompts
...
def collect_matched_probe_matrix(
    ...
    for sample in samples:
        pairs = build_prompt_pairs(
            ...
            generated_safe_prompts=generated_safe_prompts,
        )
        for pair in pairs:
...
def probe_boundaries_v5(
    cfg: Config,
    samples: list[SafetySample],
    safe_modes: list[str],
    unsafe_modes: list[str],
    generated_safe_prompt_path: str | Path | None = None,
) -> Path:
    accelerator = get_accelerator()
    cfg.validate_paths()
    torch = __import__("torch")
    out = cfg.output_path / "boundary_probes_v5.pt"
    shard_dir = cfg.output_path / "_probe_v5_shards"
    reset_shard_dir(shard_dir, accelerator)

    generated_safe_prompts = load_generated_safe_prompts(generated_safe_prompt_path)
    if accelerator.is_main_process and "generated" in safe_modes:
        print(f"[v5 probe] loaded {len(generated_safe_prompts)} generated safe prompts from {generated_safe_prompt_path}")
    ...
    if local_samples:
        ...
        layer_features, labels, records = collect_matched_probe_matrix(
            model,
            local_samples,
            cfg,
            safe_modes,
    ...
    if accelerator.is_main_process:
        ...
        for layer, arrays in sorted(merged_features.items()):
            ...
            print(f"[v5 probe] layer={layer} acc={probes[layer]['accuracy']:.4f} eps={probes[layer]['epsilon']:.4f}")
        ...
        torch.save(
            {
                ...
                "safe_modes": safe_modes,
                "unsafe_modes": unsafe_modes,
                "generated_safe_prompt_path": str(generated_safe_prompt_path) if generated_safe_prompt_path else None,
                "layers": sorted(probes),
                "probes": probes,
                "num_samples": len(samples),
                "num_prompt_records": len(merged_records),
                "records": merged_records,
                "note": "v5 uses matched safe prompts derived from each harmful instruction",
            },
            out,
        )
        (cfg.output_path / "boundary_probes_v5_meta.json").write_text(
            json.dumps(
                {
